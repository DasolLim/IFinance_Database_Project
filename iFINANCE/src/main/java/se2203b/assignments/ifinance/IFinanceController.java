...
//        // adapting between UserAccountAdapter and NonAdminAdapter (nonAdmin)
        accountGroupsController.setAdapters(new UserAccountAdapter(conn, false), new NonAdminUserAdapter(conn, false) );

        // adapting between AccountCategoryAdapter and GroupAdapter
        accountGroupsController.setAdapters(new AccountCategoryAdapter(conn, false), new GroupAdapter(conn, false) );
        // create new stage
        Stage stage = new Stage();
        stage.setScene(new Scene(toDelete));
        // add icon to the About window
